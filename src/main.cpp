...
#define HISTOGRAMSIZE 16 // 코너 좌표 주변으로 Histogram 할 사이즈
...
void GaussianFilter(Mat &Img, Mat &OutputImg, int filterSize, float sigma) { // 가우시안 필터 함수
	float *Filter = (float*)malloc(sizeof(float) * filterSize * filterSize); // 필터 생성
...
int HarrisCornerDetect(Mat &Image, HOG *Hg, Point2d *POS) {
	...
	GaussianFilter(Img, Img, 5, 1.0); // 가우시안 필터 적용 (sigma : 1.0, size : 5) 	
	...
	// SOBEL EDGE FILTER 연산
	int Filter[3] = { -1, 0, 1 }; // SOBEL FILTER
	int Mag_max = INT_MAX, Mag_min = INT_MIN; // Magnitude Normalize 를 위해 최대값 최소값 계산
	for (int i = 0; i < inputHeight; i++)
		for (int j = 0; j < inputWidth; j++) {
			int fx = 0, fy = 0;
			for (int x = 0; x < 3; x++) {
				if (i + x - 1 < 0 || i + x > inputHeight || i + 1 - x < 0 || i + 2 - x > inputHeight) // Boundary 예외 처리
					continue;
				for (int y = 0; y < 3; y++) {
					if (j + y - 1 < 0 || j + y > inputWidth || j + 1 - y < 0 || j + 2 - y > inputWidth) // Boundary 예외 처리
						continue;
					if (y != 1) fx += Filter[y] * Img.at<uchar>(i + x - 1, j + y - 1); // SOBEL FILTER 연산
					if (x != 1) fy += Filter[x] * Img.at<uchar>(i + x - 1, j + y - 1);
				}
			}
			ImgMag[i * inputWidth + j] = VECTOR_ADD(fx, fy); // 벡터 합 연산을 이용하여  magnitude 계산
			Mag_max = (Mag_max < ImgMag[i * inputWidth + j]) ? ImgMag[i * inputWidth + j] : Mag_max; // 최대, 최소값 구하기
			Mag_min = (Mag_min > ImgMag[i * inputWidth + j]) ? ImgMag[i * inputWidth + j] : Mag_min; // (Normalize시 사용)

			ImgPhase[i * inputWidth + j] = atan2(fy, fx); // arc tangent 함수를 이용하여 Phase 계산
			Ix[i * inputWidth + j] = fx; // Ix와 Iy를 포인터 변수에 저장
	...
	double max = INT_MAX; // R값의 최대값을 구하기 위한 변수
	...
	free(Iy);
	int cnt = 0;
	// 이미지를 반복문으로 돌며 R값이 Threshold보다 클때, 코너 좌표 추가
	for (int i = 0; i < inputHeight; i++)
		for (int j = 0; j < inputWidth; j++) {
			int flag = 1;
			// 각 좌표의 1픽셀 테두리를 체크하며 해당 값이 중앙의 값보다 작을때만 코너로 인정
			...
			}// R값이 R의 최대값 * Threshold 보다 클 때만 코너로 인정
			if (flag == 1 && R[i * inputWidth + j] > max * THRESHOLD) {
				POS[cnt++] = Point2d(j, i);
			}
		}
	free(R);
	return cnt; // 찾은 코너의 개수를 반환
}

void Histogram(Mat &Img, Point2d p, HOG *Hg, int BinIndex, float *Bin) { // Histogram 함수
...
{
	// 처음 실행했을때만 코너를 찾아 Histogram 한다. (그 이후부터는 현재 프레임 값을 이전 프레임에 덮어씌우기)
	if (prevcnt < 0) {
	...
	// 함수에서 포인터 변수에 값을 할당하며, 함수는 코너의 개수를 반환한다.
	...
	// prev 이미지에서 찾은 코너의 개수만큼 반복문을 실행
	for (int i = 0; i < prevcnt; i++) {
		prevMin[i] = INT_MAX; // 해당 좌표의 Histogram과 가장 유사한 Histogram 좌표를
		prevBestMatch[i] = -1; //  찾기 위해 변수 초기화
		// current 이미지의 코너 개수만큼 반복문을 돌며 비교
		for (int j = 0; j < currentcnt; j++) {
			float avg = 0; // 각 Histogram 단계별 값을 유클리드 거리를 이용해 계산
			for (int u = 0; u < 36; u++)
				avg += (prevBin[i * 36 + u] - currentBin[j * 36 + u]) * (prevBin[i * 36 + u] - currentBin[j * 36 + u]);
			if (prevMin[i] > avg) { // 유사도를 계산한 값이 최소가 되는 주소를 배열에 저장
				prevMin[i] = avg;
				prevBestMatch[i] = j;
			}
		}
	...
	for (int i = 0; i < currentcnt; i++) {
		currentMin[i] = INT_MAX; // 해당 좌표의 Histogram과 가장 유사한 Histogram 좌표를
		currentbestMatch[i] = -1; //  찾기 위해 변수 초기화
		// prev 이미지의 코너 개수만큼 반복문을 돌며 비교
		for (int j = 0; j < prevcnt; j++) {
			float avg = 0; // 각 Histogram 단계별 값을 유클리드 거리를 이용해 계산
			for (int u = 0; u < 36; u++)
				avg += (currentBin[i * 36 + u] - prevBin[j * 36 + u]) * (currentBin[i * 36 + u] - prevBin[j * 36 + u]);
			if (currentMin[i] > avg) { // 유사도를 계산한 값이 최소가 되는 주소를 배열에 저장
				currentMin[i] = avg;
	...
	// 유사도의 오차 값이 너무 높은 경우 코너로 취급하지 않음
	for (int i = 0; i < prevcnt; i++)
